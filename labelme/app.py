...
#     def merge_parallel_lines(self):
#         """Fonde segmenti contigui e paralleli in un'unica spezzata (polilinea)."""
#         import numpy as np
#         import math
#         canvas = self._canvas_widgets.canvas
#         if not canvas.shapes: 
...
#         def are_mergeable(s1, s2):
#             if len(s1.points) < 2 or len(s2.points) < 2: return False
#             p1, p2 = s1.points[0], s1.points[-1]
#             p3, p4 = s2.points[0], s2.points[-1]

#             ang1 = np.arctan2(p2.y() - p1.y(), p2.x() - p1.x()) % np.pi
#             ang2 = np.arctan2(p4.y() - p3.y(), p4.x() - p3.x()) % np.pi
#             diff = min(abs(ang1 - ang2), np.pi - abs(ang1 - ang2))
            
#             if diff > np.radians(ANGLE_EPSILON):
#                 return False

#             # Check vicinanza estremi (contiguità)
#             d1 = math.hypot(p3.x() - p1.x(), p3.y() - p1.y())
#             d2 = math.hypot(p4.x() - p1.x(), p4.y() - p1.y())
#             d3 = math.hypot(p3.x() - p2.x(), p3.y() - p2.y())
#             d4 = math.hypot(p4.x() - p2.x(), p4.y() - p2.y())
            
#             if min(d1, d2, d3, d4) < DIST_EPSILON:
#                 return True
                
#             # Check distanza ortogonale
#             mid = np.array([(p3.x() + p4.x())/2, (p3.y() + p4.y())/2])
#             if self._dist_point_to_line(mid, p1, p2) < DIST_EPSILON:
#                 return True
                
#             return False

#         # Raggruppamento transitivo (A tocca B, B tocca C -> [A,B,C])
#         groups = []
#         for i in range(len(shapes)):
#             if used[i] or len(shapes[i].points) < 2: continue
#             current_group = [shapes[i]]
#             used[i] = True
...
#                     for s_in_group in current_group:
#                         if are_mergeable(s_in_group, shapes[j]):
#                             current_group.append(shapes[j])
#                             used[j] = True
#                             added_new = True
#                             break
...
#         # Pulizia corretta usando self._docks
#         canvas.shapes = []
#         canvas.selectedShapes = []
#         if hasattr(self._docks, 'label_list'):
#             self._docks.label_list.clear()

#         # Re-inserimento Ufficiale
#         for group in groups:
#             if len(group) > 1:
#                 s = self._create_average_line(group)
#             else:
#                 s = group[0]
...
#         self.setDirty() 
#         self.statusBar().showMessage(f"Merge completato: {len(canvas.shapes)} linee/spezzate.")    
...
#         return np.abs(np.cross(p2-p1, p1-p)) / norm

#     def _create_average_line(self, group):
#         """Fonde i segmenti in un'unica SPEZZATA (polilinea), conservando i punti intermedi."""
#         all_pts = []
#         for s in group:
#             for p in s.points:
#                 all_pts.append([p.x(), p.y()])
...
#         # Calcoliamo la direzione principale solo per ordinare i punti nello spazio
#         centroid = np.mean(all_pts, axis=0)
#         _, _, vh = np.linalg.svd(all_pts - centroid)
#         direction = vh[0] 

#         # Proiettiamo i punti per capire il loro ordine da un estremo all'altro
#         projections = (all_pts - centroid) @ direction
#         sorted_indices = np.argsort(projections)
#         sorted_pts = all_pts[sorted_indices]

#         # Rimuoviamo i punti coincidenti (i vertici condivisi tra i segmenti)
#         unique_pts = []
#         for pt in sorted_pts:
#             if not unique_pts:
#                 unique_pts.append(pt)
#             else:
#                 # Se la distanza dal punto precedente è > 1.0 pixel, lo aggiungiamo
#                 # Questo elimina i duplicati creati dal tocco dei segmenti
#                 if math.hypot(pt[0] - unique_pts[-1][0], pt[1] - unique_pts[-1][1]) > 1.0:
#                     unique_pts.append(pt)

#         # shape_type="linestrip" è il formato corretto per una spezzata aperta
#         new_s = Shape(label="Linea_Merged", shape_type="linestrip")
#         for pt in unique_pts:
#             # Applichiamo lo snap per sicurezza
#             x, y = self._apply_snap(pt[0], pt[1])
#             new_s.addPoint(QtCore.QPointF(x, y))
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def _setup_actions(self) -> _Actions:
        ...
        actionMergeLines = action(
            text = self.tr("Merge Lines"), slot = self.merge_parallel_lines, shortcut="Ctrl+Shift+M", icon="merging.svg", tip=self.tr("Fonde e concatena i segmenti contigui"), enabled=False
        )
    ...
    def merge_parallel_lines(self):
        """Fonde segmenti contigui in polilinee mantenendo i vertici intermedi."""
        import math
        canvas = self._canvas_widgets.canvas
        if not canvas.shapes: 
            return

        CONTIGUITY_EPSILON = 10.0

        paths = []
        for s in canvas.shapes:
            if len(s.points) >= 2:
                paths.append({'label': s.label, 'points': list(s.points)})
        ...
        while merged_any:
            ...
            for i in range(len(paths)):
                for j in range(i + 1, len(paths)):
                    path1 = paths[i]['points']
                    path2 = paths[j]['points']

                    p1_start, p1_end = path1[0], path1[-1]
                    p2_start, p2_end = path2[0], path2[-1]

                    def dist(pa, pb): 
                        return math.hypot(pa.x() - pb.x(), pa.y() - pb.y())

                    if dist(p1_end, p2_start) < CONTIGUITY_EPSILON:
                        paths[i]['points'] = path1 + path2[1:]
                        paths.pop(j)
                        merged_any = True
                        break
                    elif dist(p1_end, p2_end) < CONTIGUITY_EPSILON:
                        paths[i]['points'] = path1 + path2[-2::-1]
                        paths.pop(j)
                        merged_any = True
                        break
                    elif dist(p1_start, p2_end) < CONTIGUITY_EPSILON:
                        paths[i]['points'] = path2 + path1[1:]
                        paths.pop(j)
                        merged_any = True
                        break
                    elif dist(p1_start, p2_start) < CONTIGUITY_EPSILON:
                        paths[i]['points'] = path2[::-1] + path1[1:]
                        paths.pop(j)
                        merged_any = True
                        break
                if merged_any:
        ...
        for path_data in paths:
            ...
            if not label or label == "Linea_Merged":
                label = self.get_next_label(prefix="L_")

            s = Shape(label=label, shape_type="linestrip")
        ...
        self.setDirty()
        self.statusBar().showMessage(f"Merge completato: raggruppati in {len(canvas.shapes)} polilinee.")    
